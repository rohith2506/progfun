...
object Huffman {
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case left :: right :: remaining => (makeCodeTree(left, right) :: remaining).sortWith((f1, f2) => weight(f1) < weight(f2))
      case _ => trees
    }
   ...
   */
    def until(x: List[CodeTree] => Boolean, y: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if(x(trees)) trees
      else until(x, y)(y(trees))
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(remaining: CodeTree)(c: Char): List[Bit] = remaining match {
        case Leaf(_, _) => List()
